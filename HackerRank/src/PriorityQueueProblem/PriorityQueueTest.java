...
class Priorities implements Comparator<Student>{
    ...
    public List<Student> getStudents(List<String> events){
    	...
    	Queue<Student> pq = new PriorityQueue<Student>(10,pr); // pr is an object of class priority which override the compare method
    	Queue<Student> pq1 = new PriorityQueue<Student>(10,pr);  
    	...
    	for( String event : events ) {
    		if(event.equals("SERVED")) {
    			...
    			studentDetails = event.split(" ");
    			pq.add(new Student(Integer.parseInt(studentDetails[3]), studentDetails[1],Double.parseDouble(studentDetails[2])));
     		}
    	}

    	// use the other priority queue to sort  and then return
        ...
        return (new ArrayList<Student>(pq1));
    }
